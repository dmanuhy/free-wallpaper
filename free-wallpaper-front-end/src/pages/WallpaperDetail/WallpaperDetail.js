        ...
        <> {wallpaperDetail &&
            <div className="wallpaper-detail content-width-padding content-height-padding">
                <div className="wallpaper-detail-main">
                    <img onLoad={(event) => setContentMaxHeight(event.target.clientHeight)} ref={imageRef} className="wallpaper-detail-img" src={wallpaperDetail.imageUrl} alt={wallpaperDetail._id} />
                    <div className="wallpaper-detail-content d-flex flex-column justify-content-between p-2">
                                                                    ...
                                                                    commentReplyDisplay.indexOf(item._id) > -1 && item.replies.length > 0 && item.replies.sort((a, b) => a.date - b.date).map((item, index) => {
                                                                        return (
                                                                            <div className="d-flex gap-2 py-1">
                                                                                ...
                                                                                <div className="d-flex flex-column">
                                                                                    <span className="wallpaper-detail-reply-body pt-1 text-wrap">{item.body}</span>
                                                                                    <span className="wallpaper-detail-reply-time">{moment(item.date).fromNow()}</span>
                                                                                </div>
                                                                            </div>
                                                                        )
                                                                    })
                        ...
                        <div className="wallpaper-detail-bottom-content">
                            {user && user._id ?
                                <CommentInput className="wallpaper-detail-comment-input w-100" placeholder="Add a comment ..." userID={user._id} wallpaperID={wallpaperDetail._id} wallpaperDetail={wallpaperDetail} setWallpaperDetail={setWallpaperDetail} />
                                :
