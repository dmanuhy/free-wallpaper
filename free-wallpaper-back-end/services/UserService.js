...
const getUserNotificationService = async (id) => {
  try {
    const userNotification = await db.user.findOne({ _id: id }).select("_id notifications");
    return {
      status: 200,
...
const getUserLikedWallpaperService = async (id) => {
  try {
    const userNotification = await db.user.findOne({ _id: id }).select("_id liked");
    return {
      status: 200,
      data: userNotification,
    };
...
const updateUserLikedWallpaperService = async (userId, wallpaperId, userName, ownerId) => {
  try {
    let action = "";
    const userLikedWallpaper = await db.user.findOne({ _id: userId }).select("_id liked");
...
const sendNotificationService = async (ownerId, body, link = "") => {
  const userNotification = await db.user
    .findOneAndUpdate(
      { _id: ownerId },
...
const markReadedNotificationService = async (userId, notificationId) => {
  const userNotification = await db.user.findOne({ _id: userId }).select("_id notifications");
  if (userNotification) {
    const matchedNotificationIndex = userNotification.notifications.findIndex(
